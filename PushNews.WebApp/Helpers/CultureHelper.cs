...
using Culturas = PushNews.WebApp.App_LocalResources.Culturas;
...
namespace PushNews.WebApp.Helpers
{
    public class CultureHelper
    {
        ...
        private static readonly Dictionary<string,string> cultures = new Dictionary<string,string> {
            {"es-ES", Culturas.EspanolEspana}, // first culture is the DEFAULT
            {"en-US", Culturas.InglesEEUU},
            // Añadir culturas adicionales aquí. Deben existir en la lista validCultures y además
            // deben tener un nombre internacionalizado en el archivo de recursos Culturas.
        };

        public static List<SelectListItem> ListaCulturas 
        {
            get 
            {
                return cultures.Select(c => new SelectListItem() { Value = c.Key, Text = c.Value }).ToList();
            }
        }
        ...
        public static string GetImplementedCulture(string name)
        {
            ...
            if (cultures.Where(c => c.Key.Equals(name, StringComparison.InvariantCultureIgnoreCase)).Count() > 0)
        ...
        {
            return cultures.ElementAt(index: 0).Key; // return Default culture
        }
        ...
        public static string GetNeutralCulture(string name)
        {
            ...
            return name.Contains(value: "-") ? name.Split(separator: new char[] { '-' })[0] : name;
        }
    }
}